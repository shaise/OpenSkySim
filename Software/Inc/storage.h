...
// our storage struct contains all data that has to be stored on flash - MUST BE 4 bytes aligned!!!
typedef struct {
    // version id
    uint8_t version;
    uint8_t align1;
    ...
    uint8_t fail_safe_state[8]; // to preserve memory - only 8 bits per channel

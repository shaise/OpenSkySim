...
void frsky_configure(void) {
    ...
    // normal config
    // go back to rx after transmission completed (0x0C would be stay idle)
    cc25xx_set_register(MCSM1    , 0x0F);
    cc25xx_set_register(MCSM0    , 0x18);
    // on 251x this has to be exactly our size
    cc25xx_set_register(PKTLEN   , FRSKY_PACKET_LENGTH);
    cc25xx_set_register(PKTCTRL0 , 0x05);
    cc25xx_set_register(PA_TABLE0, 0xFF);
    // D4R-II seems to set 0x68 here ?! instead of 0x08
...
void frsky_autotune(void) {
    ...
    int8_t rssi;
    int8_t max_rssi = -120;
    int8_t max_rssi_start, max_rssi_end;

    uint8_t lqi;
    uint8_t min_lqi = 255;
    int8_t min_lqi_start, min_lqi_end;
    ...
    while (state != 5) {
        ...
        while ((!timeout_timed_out()) && (!done)) {
            ...
            cc25xx_process_packet(&frsky_packet_received,
            ...
            if (frsky_packet_received) {
                ...
                if (FRSKY_VALID_PACKET_BIND(frsky_packet_buffer)) {
                    ...
                    rssi =  (int8_t)frsky_packet_buffer[FRSKY_PACKET_BUFFER_SIZE-2];
                    if (rssi > max_rssi + 1) {
                        max_rssi = rssi;
                        max_rssi_start = max_rssi_end = storage.frsky_freq_offset;
                    } else if (rssi >= max_rssi - 1) {
                        max_rssi_end = storage.frsky_freq_offset;
                    }

                    lqi = frsky_packet_buffer[FRSKY_PACKET_BUFFER_SIZE-1];
                    if (lqi < min_lqi - 3) {
                        min_lqi = lqi;
                        min_lqi_start = min_lqi_end = storage.frsky_freq_offset;
                    } else if (lqi < min_lqi + 3) {
                        min_lqi_end = storage.frsky_freq_offset;
                    }
    ...
    fscal0_calc = ((int16_t)min_lqi_start + (int16_t)min_lqi_end)/2;
    ...
    debug("\nfrsky: max rssi ");
    debug_put_int8(max_rssi);
    debug("  at ofssets ");
    debug_put_int8(max_rssi_start);
    debug(" - ");
    debug_put_int8(max_rssi_end);

    debug("\nfrsky: min lqi ");
    debug_put_uint8(min_lqi);
    debug("  at ofssets ");
    debug_put_int8(min_lqi_start);
    debug(" - ");
    debug_put_int8(min_lqi_end);
    ...
    // store new value
    storage.frsky_freq_offset = (int8_t)fscal0_calc;
...
void frsky_configure_address(void) {
    ...
    // freq offset
    cc25xx_set_register(FSCTRL0, 20 /*storage.frsky_freq_offset*/);
    ...
    cc25xx_set_register(PKTCTRL1,
                        CC2500_PKTCTRL1_APPEND_STATUS |
                        CC2500_PKTCTRL1_CRC_AUTOFLUSH |
    ...
    while (1) {
        if (timeout_timed_out()) {
            ...
            if (hopcount++ >= FRSKY_COUNT_RXSTATS) {
                ...
                debug("%, ch3: ");
                debug_put_uint16(show_data);
                debug_put_newline();
 
                // for testing
                // debug_put_uint16((uint16_t)(((
                //                  frsky_packet_buffer[11] & 0x0F) << 8 |
                //                  frsky_packet_buffer[8])));
        ...
        if (frsky_packet_received) {
            if (!failsafe_programming_in_progress()) {
                led_red_off();
            }
            ...
            if (FRSKY_VALID_PACKET(frsky_packet_buffer)) {
                ...
                while (!timeout2_timed_out()) {
                    // wait for 0.5ms to elapse, set by process packet
                }
                ...
                //debug_put_int8(timeout_timed_out());
...
void frsky_update_ppm(void) {
    ...
    */
    ...
    show_data = channel_data[2];

    // exit failsafe mode
    failsafe_exit();

    // update hid
    hid_update(channel_data);

    // check for failsafe programming
    failsafe_programming_check(channel_data);
}
...
void frsky_send_telemetry(uint8_t telemetry_id) {
    ...
    // ADC channels
    frsky_packet_buffer[3] = 0;
    frsky_packet_buffer[4] = 0;
    // RSSI
    ...
    timeout2_delay_100us(50); // takes at least 4 ms to transmit, no need t poll completion till then.
